...
<%
	...
	if(reportTitle.isEmpty() == true || reportContent.isEmpty() == true){
		PrintWriter script = response.getWriter();
		script.println("<script>");
	...
	String to = "tmdwns1101@naver.com";
	String subject = "강의평가 신고 사항";  //이메일 제목
	String content = "신고자: " + userID +
					 "<br>제목: " + reportTitle +
					 "<br>신고내용: " + reportContent;
	...
	PrintWriter script = response.getWriter();
	script.println("<script>");
	script.println("alert('신고 접수가 완료되었습니다.');");
	script.println("history.back();");
	script.println("</script>");
	script.close();
	return;
